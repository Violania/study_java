...
public class GameFrame extends Frame implements Game.GameEventListener, ActionListener {
    ...
    public void onStateChanged() {
        ...
        if (!players.isEmpty()) {
            moneyLabel.setText("내 칩: " + formatMoney(players.get(0).money));
        }

        potLabel.setText("팟: " + formatMoney(game.accumulatedPot));

        if (centerPotLabel != null) {
            centerPotLabel.setText("팟 " + formatMoney(game.accumulatedPot));
        }
    ...
    public void refreshTableAndCards() {
        ...
        if (players.size() >= 4) {
            ...
            centerPotLabel = new Label("팟 " + formatMoney(game.accumulatedPot), Label.CENTER);
    ...
    private Panel makePlayerPanel(final Player player, boolean compact) {
        ...
        String nameText = player.name + playerState + " (" + formatMoney(player.money) + ")";
        final Label name = new Label(nameText, Label.CENTER);
