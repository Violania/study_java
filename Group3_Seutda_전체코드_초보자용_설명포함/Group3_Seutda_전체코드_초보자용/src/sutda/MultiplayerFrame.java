...
public class MultiplayerFrame extends Frame implements SeutdaClient.Listener, ActionListener {
    ...
    public void onState(final SeutdaClient.GameState state) {
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                potLabel.setText("팟: " + formatMoney(state.pot));

                SeutdaClient.PlayerState me = findMyPlayerState(state);

                if (me != null) {
                    moneyLabel.setText("내 칩: " + formatMoney(me.money));
                }
    ...
    public void onResult(final String summary, final String result) {
        EventQueue.invokeLater(new Runnable() {
            ...
            public void run() {
                ...
                SeutdaClient.PlayerState me = findMyPlayerState(currentState);
    ...
    private void renderTable(SeutdaClient.GameState state) {
        ...
        centerPotLabel = new Label("팟 " + formatMoney(state.pot), Label.CENTER);
        ...
        Label betInfo = new Label(state.phase + "차 베팅 / 기준 " + formatMoney(state.targetBet), Label.CENTER);
    ...
    private Panel makePlayerPanel(SeutdaClient.PlayerState player, boolean compact) {
        ...
        Label name = new Label(player.name + stateText + "  " + formatMoney(player.money), Label.CENTER);
        if (player.id == myId) {
        ...
        String hand = "베팅: " + formatMoney(player.currentBet);
    ...
    /** 현재 상태에서 내 플레이어 정보를 찾습니다. */
    private SeutdaClient.PlayerState findMyPlayerState(SeutdaClient.GameState state) {
        if (state == null) {
            return null;
        }

        for (int i = 0; i < state.players.size(); i++) {
            SeutdaClient.PlayerState player = state.players.get(i);
    ...
    /** 카드 번호에 맞는 이미지 파일 경로를 만듭니다. */
    private static String getCardImagePath(Card card) {
        String monthText;

        if (card.month < 10) {
            monthText = "0" + card.month;
        } else {
            monthText = String.valueOf(card.month);
        }

        return "/images/card_" + monthText + "_" + card.number + ".png";
    }
    ...
    private class NetworkCardView extends Canvas {
        ...
        NetworkCardView(int cardNumber, boolean revealed, int width, int height) {
            ...
            if (this.revealed) {
                ...
                image = loadImage(getCardImagePath(card));
            } else {
